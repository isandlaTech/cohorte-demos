...
class MonitorBasic(object):
    ...
    def start_isolate(self, name, kind, level, sublevel, bundles=None,
        ...
        if not uid:
            # uid = uuid.uuid4()
            uid = "{0}-{1}".format(self._node_uid, name)

        # check if the isolate is not starting
        try:
            state = self._status.get_state(uid)
            if state in (fsm.ISOLATE_STATE_GONE, fsm.ISOLATE_EVENT_STOPPING):
                raise KeyError("Ignore stopping isolates")
        except KeyError:
            # Isolate is not (considered) running
            pass
        else:
            # Isolate is already running, so nothing to do
            _logger.debug("Isolate %s is already running. Ignore start order",
                          uid)
            return True
