    ...
    def load_repositories(self):
        ...
        with self.__lock:
            ...
            for repository in self._repositories:
                #_logger.error("+++++++++ load_repository : %s", repository)
                for artifact in repository.walk():
                    #_logger.error("____>_____ reading artifcat: %s", artifact)
                    # ######### added by: Bassem D. (issue #)
                    sys.stdout.write('.')
                    sys.stdout.flush()
                    # #########
                    self.add_artifact(artifact)
                    #_logger.error("____<_____ read artifact: %s", artifact)

    def __initial_loading(self):
        """
        Initial repository loading
        """
        _logger.error("+++++++++ ipopo __initial_loading")
        self.load_repositories()
        _logger.error("+++++++++ ipopo setting controller")
        self._controller = True
        _logger.error("+++++++++ ipopo controller True")
